...
function pk_bookmark_add_meta_boxes() {
    add_meta_box('pk_bookmark_headers', 'URL', 'pk_bookmark_metabox', 'bookmark', 'normal', 'high');
}
...
function pk_bookmark_metabox($post) {
    ...
    echo '<table>';
	    echo '<tr><td><b>URL:</b></td><td><input type="text" name="pk_bookmark_link_url" value="'.($custom ? htmlspecialchars(array_pop($custom['link_url'])) : '').'" style="width: 400px"/></td></tr>';
    echo '</table>';
